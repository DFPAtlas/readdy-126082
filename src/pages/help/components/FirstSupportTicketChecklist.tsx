...
import { FIRST_SUPPORT_TICKET_CHECKLIST, FIRST_SUPPORT_TICKET_CHECKLIST_STORAGE_KEY } from '../helpContent';

export default function FirstSupportTicketChecklist() {
  const [checked, setChecked] = useState<boolean[]>(() => {
    try {
      const raw = localStorage.getItem(FIRST_SUPPORT_TICKET_CHECKLIST_STORAGE_KEY);
      if (raw) {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed)) {
          return FIRST_SUPPORT_TICKET_CHECKLIST.map((_, i) => Boolean(parsed[i]));
        }
      }
    } catch {
      /* ignore corrupt storage */
    }
    return FIRST_SUPPORT_TICKET_CHECKLIST.map(() => false);
  });
  const [confirmReset, setConfirmReset] = useState(false);

  useEffect(() => {
    try {
      localStorage.setItem(FIRST_SUPPORT_TICKET_CHECKLIST_STORAGE_KEY, JSON.stringify(checked));
    } catch {
  ...
  const reset = () => {
    setChecked(FIRST_SUPPORT_TICKET_CHECKLIST.map(() => false));
    setConfirmReset(false);
  };

  const completedCount = checked.filter(Boolean).length;
  const allDone = completedCount === FIRST_SUPPORT_TICKET_CHECKLIST.length;

  return (
    <section id="first-support-ticket-checklist" className="bg-background-100 border border-background-200/60 rounded-lg p-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-base font-heading font-semibold text-foreground-50">Your First Support Ticket Checklist</h2>
            <p className="text-xs text-foreground-500 mt-0.5">A training aid to help you handle a support ticket safely end to end.</p>
          </div>
        </div>
        <span className="text-xs text-foreground-500 whitespace-nowrap">{completedCount}/{FIRST_SUPPORT_TICKET_CHECKLIST.length} complete</span>
      </div>
      ...
      <div className="mt-4">
        <div className="h-1.5 bg-background-200/60 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${Math.round((completedCount / FIRST_SUPPORT_TICKET_CHECKLIST.length) * 100)}%` }}
          ></div>
        </div>
      </div>

      <ul className="mt-4 space-y-1">
        {FIRST_SUPPORT_TICKET_CHECKLIST.map((item, i) => (
          <li key={item}>
      ...
      <div className="mt-4 pt-4 border-t border-background-200/40 flex items-center justify-between gap-3 flex-wrap">
        <p className="text-xs text-foreground-600">Progress is saved on this device only and does not change any ticket records.</p>
        {confirmReset ? (
      ...
      {allDone && (
        <div className="mt-3 flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/25">
          ...
          <p className="text-xs text-emerald-200">All first-ticket steps complete — great work.</p>
        </div>
      )}
    </section>
  );
