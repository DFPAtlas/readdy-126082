import { SUPPORT_TICKET_JOURNEY } from '../helpContent';

interface Props {
  onSelect: (articleId: string) => void;
}

export default function SupportTicketJourney({ onSelect }: Props) {
  return (
    <section className="bg-background-100 border border-background-200/60 rounded-lg p-5">
      <div className="flex items-start gap-3 mb-4">
        <span className="w-8 h-8 rounded-lg bg-accent-500/10 text-accent-400 flex items-center justify-center shrink-0">
          <i className="ri-ticket-2-line text-sm w-4 h-4 flex items-center justify-center"></i>
        </span>
        <div>
          <h2 className="text-base font-heading font-semibold text-foreground-50">Support Ticket Journey</h2>
          <p className="text-xs text-foreground-500 mt-0.5">Select a stage to open its guide.</p>
        </div>
      </div>

      <ol className="flex flex-col gap-1" aria-label="Support Ticket Journey">
        {SUPPORT_TICKET_JOURNEY.map((stage, i) => (
          <li key={stage.id} className="flex items-start gap-3">
            <div className="flex flex-col items-center shrink-0 pt-1">
              ...
              {i < SUPPORT_TICKET_JOURNEY.length - 1 && (
