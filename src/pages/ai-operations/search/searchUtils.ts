...
// filtering and quick-navigation metadata.
// ============================================================================
...
export function searchRecords(query: string): AiGlobalSearchResult[] {
...
export function queryResults(query: string): AiGlobalSearchResult[] {
  if (normalize(query)) return searchRecords(query);
  return [...searchIndex].sort(
    (a, b) =>
...
export function applyFilters(list: AiGlobalSearchResult[], f: SearchFilters): AiGlobalSearchResult[] {
  return list.filter((r) => {
    ...
    if (f.site) {
      if (f.site === 'group') {
        // Group-wide records are represented as 'group' or null across registries.
