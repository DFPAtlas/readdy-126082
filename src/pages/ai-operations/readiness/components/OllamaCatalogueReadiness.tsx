...
export default function OllamaCatalogueReadiness() {
    ...
    <section className="bg-background-100 border border-background-200/60 rounded-lg">
      <div className="px-4 py-3 border-b border-background-200/60 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-label font-semibold text-foreground-200 uppercase tracking-wide">Phase 3 — Local Ollama Catalogue</h3>
          <span className="inline-flex items-center gap-1 text-[10px] font-label text-red-400 bg-red-500/10 border border-red-500/25 rounded-full px-2 py-0.5 whitespace-nowrap">catalogue only · no inference</span>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 p-4">
        {ollamaCatalogueReadiness.map((item) => {
      ...
      <div className="px-4 py-3 border-t border-background-200/60 flex items-center gap-2.5 flex-wrap">
        <StatusPill tone={freshness.tone} label={freshness.label} />
        <p className="text-xs text-foreground-500">
          {comparison
            ? `Relayed catalogue: ${comparison.totalCatalogueModels} model(s) · ${comparison.registeredPresent} registered+present · ${comparison.registeredMissing} missing · ${comparison.presentNotRegistered} unregistered · ${comparison.remoteCloud} remote/cloud.`
            : 'No relayed catalogue yet — a genuine bridge catalogue relay is required.'}{' '}
          <strong className="text-foreground-300">Production Enabled = 0 · Overall = NO-GO.</strong>
        </p>
      </div>
    </section>
  );
