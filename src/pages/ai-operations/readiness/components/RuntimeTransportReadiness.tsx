...
export default function RuntimeTransportReadiness() {
  return (
    <section className="bg-background-100 border border-background-200/60 rounded-lg">
      <div className="px-4 py-3 border-b border-background-200/60 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-label font-semibold text-foreground-200 uppercase tracking-wide">Phase 3 — Private Runtime Transport</h3>
          <span className="inline-flex items-center gap-1 text-[10px] font-label text-amber-400 bg-amber-500/10 border border-amber-500/25 rounded-full px-2 py-0.5 whitespace-nowrap">dry-run only</span>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 p-4">
        {runtimeTransportReadiness.map((item) => {
      ...
      <div className="px-4 py-3 border-t border-background-200/60 flex items-center gap-2.5">
        ...
        <p className="text-xs text-foreground-500">
          <strong className="text-foreground-300">Transport is the only thing being verified.</strong> A successful dry-run probe proves cloud → HAL → cloud control-message delivery, but it does <strong className="text-foreground-300">not</strong> satisfy the Master Kill Switch, Production Enabled, Site/Agent/Approval/Policy gates, or Execution Dispatch. Execution transport stays <strong className="text-red-400">Dry-Run Only</strong> and dispatch remains <strong className="text-red-400">Not Started</strong>.
        </p>
      </div>
    </section>
  );
