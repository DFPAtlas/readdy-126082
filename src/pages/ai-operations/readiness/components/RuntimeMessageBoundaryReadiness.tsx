...
export default function RuntimeMessageBoundaryReadiness() {
  return (
    <section className="bg-background-100 border border-background-200/60 rounded-lg">
      <div className="px-4 py-3 border-b border-background-200/60 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-label font-semibold text-foreground-200 uppercase tracking-wide">Phase 3 — Runtime Message Boundary</h3>
          <span className="inline-flex items-center gap-1 text-[10px] font-label text-red-400 bg-red-500/10 border border-red-500/25 rounded-full px-2 py-0.5 whitespace-nowrap">verification only</span>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 p-4">
        {runtimeMessageBoundary.map((item) => {
      ...
      <div className="px-4 py-3 border-t border-background-200/60 flex items-center gap-2.5">
        ...
        <p className="text-xs text-foreground-500">
          <strong className="text-foreground-300">Verification is Ready; processing is not.</strong> The signed callback channel verifies and records, but n8n callback connectivity stays <strong className="text-foreground-300">Not Verified</strong> until a genuine signed handshake arrives, Runtime Result Processing remains <strong className="text-foreground-300">Not Started</strong>, and <strong className="text-red-400">Run State Mutation is Disabled</strong> — callbacks never touch <span className="font-mono">ai_runs</span> or orchestration execution state.
        </p>
      </div>
    </section>
  );
