...
export default function RuntimeBridgeReadiness() {
  return (
    <section className="bg-background-100 border border-background-200/60 rounded-lg">
      <div className="px-4 py-3 border-b border-background-200/60 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-label font-semibold text-foreground-200 uppercase tracking-wide">Phase 3 — Private Runtime Bridge</h3>
          <span className="inline-flex items-center gap-1 text-[10px] font-label text-red-400 bg-red-500/10 border border-red-500/25 rounded-full px-2 py-0.5 whitespace-nowrap">outbound · no execution</span>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 p-4">
        {runtimeBridgeReadiness.map((item) => {
      ...
      <div className="px-4 py-3 border-t border-background-200/60 flex items-center gap-2.5">
        ...
        <p className="text-xs text-foreground-500">
          <strong className="text-foreground-300">Framework is Ready; connectivity is not verified.</strong> Code existence is not enough — the bridge stays <strong className="text-foreground-300">Not Verified</strong> until a genuine authenticated outbound handshake produces a valid node + heartbeat. Local n8n / Ollama health remain <strong className="text-foreground-300">Not Configured</strong> until the bridge reports them, and <strong className="text-red-400">Execution Transport is Disabled</strong> — the bridge is transport only and never owns execution authority.
        </p>
      </div>
    </section>
  );
