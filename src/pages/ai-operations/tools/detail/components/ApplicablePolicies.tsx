import { getPoliciesForTool } from '@/pages/ai-operations/security/selectors';
import PolicyLinkList from '@/pages/ai-operations/security/components/PolicyLinkList';

export default function ApplicablePolicies({ connectionId }: { connectionId: string }) {
  const policies = getPoliciesForTool(connectionId);
  ...
  return (
    <section className="bg-background-100 border border-background-200/60 rounded-lg">
      ...
      <div className="px-4 py-3">
        <PolicyLinkList policies={policies} emptyMessage="No policies explicitly target this connection." />
      </div>
    </section>
  );
}