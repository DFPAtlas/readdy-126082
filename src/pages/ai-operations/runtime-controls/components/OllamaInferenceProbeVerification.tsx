...
import {
  queueOllamaInferenceProbe,
  getOllamaInferenceProbeStatus,
  deriveOllamaProbeStatus,
  isOllamaProbeTerminal,
  formatMs,
  OLLAMA_PROBE_STATUS_META,
  OLLAMA_PROBE_NODE_KEY,
  OLLAMA_PROBE_MODEL,
  OLLAMA_PROBE_PROMPT_ID,
  OLLAMA_PROBE_MODE,
  OLLAMA_PROBE_MAX_TOKENS,
  OLLAMA_PROBE_TIMEOUT_SECONDS,
  OLLAMA_PROBE_EXPECTED_OUTPUT,
  type OllamaProbeStatusResult,
  type OllamaProbeStatusEntry,
  type OllamaProbeStatus,
...
export default function OllamaInferenceProbeVerification() {
  const { role } = useAuth();
  ...
  const [status, setStatus] = useState<OllamaProbeStatusResult | null>(null);
  ...
  async function refreshStatus() {
    setLoading(true);
    setError(null);
    const res = await getOllamaInferenceProbeStatus();
    if (res.error) {
      setError(res.error);
    } else if (res.data) {
      setStatus(res.data);
  ...
  async function pollUntilTerminal(correlationId: string) {
    ...
    while (pollActiveRef.current && attempts < 12) {
      ...
      const res = await getOllamaInferenceProbeStatus(correlationId);
      if (res.error) {
        setError(res.error);
        break;
      }
      if (res.data) {
        ...
        if (latest && isOllamaProbeTerminal(deriveOllamaProbeStatus(latest))) break;
      }
      attempts += 1;
    }
    pollActiveRef.current = false;
  ...
  async function handleRunProbe() {
    ...
    setSending(true);
    setError(null);
    const queued = await queueOllamaInferenceProbe(OLLAMA_PROBE_NODE_KEY);
    if (queued.error) {
  ...
  const latest: OllamaProbeStatusEntry | undefined = status?.probes?.[0];
  const latestStatus: OllamaProbeStatus = latest ? deriveOllamaProbeStatus(latest) : 'pending';
  const meta = OLLAMA_PROBE_STATUS_META[latestStatus];
  ...
  return (
    <section className="bg-background-100 border border-background-200/60 rounded-lg">
      <div className="px-4 py-3 border-b border-background-200/60 flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-accent-500/10 text-accent-400 flex items-center justify-center shrink-0">
            <i className="ri-cpu-line w-4 h-4 flex items-center justify-center"></i>
          </div>
          <div>
            <h3 className="text-sm font-label font-semibold text-foreground-200 uppercase tracking-wide">Controlled Ollama Sandbox Probe</h3>
            <p className="text-xs text-foreground-500 mt-0.5">One fixed harmless local Ollama diagnostic through HAL — no arbitrary inference.</p>
          </div>
        </div>
      ...
      <div className="px-4 py-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-background-200/60 rounded-md overflow-hidden">
          <ConfigStat label="Node" value={nodeName} />
          <ConfigStat label="Model" value={OLLAMA_PROBE_MODEL} mono />
          <ConfigStat label="Prompt ID" value={OLLAMA_PROBE_PROMPT_ID} mono />
          <ConfigStat label="Mode" value={OLLAMA_PROBE_MODE} mono />
          <ConfigStat label="Max generation" value={`${OLLAMA_PROBE_MAX_TOKENS} tokens`} />
          <ConfigStat label="Timeout" value={`${OLLAMA_PROBE_TIMEOUT_SECONDS} seconds`} />
          <ConfigStat label="Expected output" value={OLLAMA_PROBE_EXPECTED_OUTPUT} mono />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-px bg-background-200/60">
        ...
        <SummaryStat label="Model" value={OLLAMA_PROBE_MODEL} mono />
        <SummaryStat label="Prompt ID" value={OLLAMA_PROBE_PROMPT_ID} mono />
        <SummaryStat
          label="Probe Status"
      ...
      <div className="px-4 py-3 border-t border-background-200/60">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            ...
            {sending ? 'Queueing…' : 'Run Sandbox Ollama Probe'}
          </button>
          <button
        ...
        {latest && (
          <div className="mt-3 space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
              ...
              />
              <DetailStat label="Output" value={latest.output || '—'} mono />
              <DetailStat label="Local inference latency" value={formatMs(latest.latencyMs)} />
              <DetailStat label="End-to-end round trip" value={formatMs(latest.roundTripMs)} />
              <DetailStat label="Expiry" value={formatTime(latest.expiresAt)} />
              <DetailStat label="Node" value={latest.expectedNodeKey || OLLAMA_PROBE_NODE_KEY} mono />
      ...
      <div className="px-4 py-3 border-t border-background-200/60 flex items-center gap-2.5">
        ...
        <p className="text-xs text-foreground-500">
          <strong className="text-foreground-300">Controlled diagnostic inference only — no agent, workflow, tool or business action executed.</strong>{' '}
          The probe always uses the fixed model and prompt; it never sends arbitrary prompts, and success is only shown when the backend returns a verified match. Execution remains <strong className="text-red-400">BLOCKED</strong>.
        </p>
      </div>
    </section>
  );
...
function ResultBanner({ latest }: { latest: OllamaProbeStatusEntry }) {
  const status = deriveOllamaProbeStatus(latest);
    ...
    return (
      <div className="bg-emerald-500/10 border border-emerald-500/25 rounded-md px-3 py-3">
        <div className="flex items-center gap-2.5 mb-1.5">
          ...
          <p className="text-xs text-emerald-300/90 font-semibold">Ollama Sandbox Probe Verified</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1">
          <div className="flex items-center justify-between gap-2">
            <span className="text-[11px] text-emerald-400/70">Output</span>
            <span className="text-xs text-emerald-100 font-mono">{latest.output || '—'}</span>
          </div>
          <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-xs text-emerald-100 font-mono">{latest.model || OLLAMA_PROBE_MODEL}</span>
          </div>
          <div className="flex items-center justify-between gap-2">
        ...
        <p className="text-[11px] text-emerald-400/70 mt-2">Controlled diagnostic inference only — no agent, workflow, tool or business action executed.</p>
      </div>
    );
  }
  ...
  if (status === 'rejected') {
    return (
      <div className="flex items-center gap-2.5 bg-red-500/10 border border-red-500/25 rounded-md px-3 py-2.5">
        ...
        <p className="text-xs text-red-300/90">Probe rejected — not verified. No execution was performed.</p>
      </div>
    );
  }
  ...
  if (status === 'failed') {
    return (
      <div className="flex items-center gap-2.5 bg-red-500/10 border border-red-500/25 rounded-md px-3 py-2.5">
        <i className="ri-close-circle-line text-red-400 w-4 h-4 flex items-center justify-center shrink-0"></i>
        <p className="text-xs text-red-300/90">Probe returned a result that was not verified — output did not match the expected sentinel.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2.5 bg-amber-500/10 border border-amber-500/25 rounded-md px-3 py-2.5">
      ...
      <p className="text-xs text-amber-300/90">Probe {status} — awaiting a signed verified result from HAL. No arbitrary inference is performed.</p>
    </div>
  );
}
