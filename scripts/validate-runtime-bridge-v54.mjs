...
assert.match(watchdog, /if systemctl try-restart "\$service_name"; then/);
assert.match(watchdog, /restart completed for \$service_name/);
assert.match(watchdog, /restart failed for \$service_name/);
assert.doesNotMatch(watchdog, /systemctl try-restart "\$service_name"\n\s*exit 1/);
assert.doesNotMatch(watchdog, /nanosecond|%N/i);
assert.match(envExample, /DFP_BRIDGE_LIVENESS_INTERVAL_SECONDS=1/);
assert.match(watchdogService, /^StartLimitIntervalSec=0$/m);
